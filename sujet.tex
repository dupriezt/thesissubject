...
\emph{Note: Pharo\footnote{http://www.pharo.org} is a dynamic object-oriented programming language with strong reflective capabilities. It is coupled with an IDE made of pharo objects that can be directly sent messages from the source code. This last trait makes it very suitable for the development of tools for developers.}

\gp{I gathered experience on research in...? }
I already did some research in the debugging field during my M2 internship and research year (in progress) conducted at the Rmod team - Inria Lille Nord-Europe, and Soft team - Vrije Universiteit Brussel. 
...
HaltManager\footnote{Available on github https://github.com/dupriezt/HaltManager} is a package I developed for the Pharo IDE (for the Pharo programming language). HaltManager shows  all the breakpoints and halt messages (halt messages are normal messages that, when sent to an object, stop the execution and open a debugger like a breakpoint. The main difference with breakpoints is that halt messages are normal source code, while breakpoints are an IDE feature) of the system, and allows one to toggle them on/off with a simple click (see image below). Halt messages can also be toggled via automatic source code rewriting. The deactivation/reactivation feature is available in all code browsers of the IDE, not only in the HaltManager window.
...
I published an analysis of missing debugging features and prototypes for new debugging tools as a position paper named \emph{Analysis and exploration for new generation debuggers}\footnote{https://doi.org/10.1145/3139903.3139910} in the IWST workshop of ESUG 2017.
...
\gp{Put your publication in the bib file and then add this as a normal reference, it looks more serious :)}

\paragraph{Expression evaluation recorder.}
\gp{Tell it as if it was an experiment. What did you learn from it?}
This tool is a prototype built with the goal of allowing the developer to access the past state of an execution. It allows one to instrument a method by reflectively modifying its bytecode so that whenever one of its expressions is evaluated, the result is copied in a dictionary that can later be accessed by the developer.

\paragraph{Invariant checker.}
\gp{Tell it as if it was an experiment. What did you learn from it?}
Based on the idea expressed by Arya et all~\cite{Arya17a}, this tool is a prototype taking as input an expression (for example returning whether a particular invariant is satisfied) and some code. It first executes the expression, stores its value, then runs the provided code while re-executing the expression after each step. If the expression evaluates to a different value than the initial one, the execution is stopped and a debugger opened on the statement that caused this value change. While having limitations, it allows a developer to simply enter an invariant and be shown the point of the program it stops being true.

\paragraph{Debugging of concurrent programs (in progress).}
\gp{Tell it as if it was an experiment. What did you learn from it?}
As part of an ongoing internship at the Soft team of the Vrije Universiteit Brussel, I am collaborating with 1) a project to design a formal semantics for a debugger of concurrent programs with the objective to guide the implementation of an actual debugger and be re-purposed for different concurrency models and 2) a remote debugger for big data clusters.
...
We plain to aboard this research by exploring three different axis: back-in-time debugging, execution comparison and debugging specific programming languages.

Each of these axis represents a promising, but challenging to precisely define and implement, debugging feature. However, the objective is not simply to develop new debugging feature in a vacuum, but to introduce them in real development scenarios. For this reason, we intend organize our work in agile cycles of ~6 months each. Each cycle will comprise the following steps:
\begin{enumerate}
...
A back in time debugger is a debugger that can go backwards in an execution. This is especially useful to answer questions like \emph{where does this value come from?} or to find the cause of a bug when it is no longer on the execution stack. Implementing a back-in-time debugger is challenging by the intrinsic forward nature of executions (variables get overwritten, method calls that return are forgotten, unused memory gets reclaimed...). The two main existing techniques to implement a back in time debugger are 1) storing the state of the execution to go back to it later and 2) re-executing the program up until the desired point. These axis show the trade-off between memory usage and time taken to re-execute as a central consideration for the design of a back in time debugger.

Besides this consideration there are a few key challenges like limiting the runtime overhead, having an acceptable memory footprint, defining the scope of what should be accessible (whole system?, some objects defined in advance by the developer?) and handling the interactions with the outside world (network requests, OS calls...) happening during the execution. \gp{Too long sentence}
...
The main challenges for this feature are: what is a definition of \emph{difference} between two executions that would be useful for debugging, and how to recognise matching objects between the two executions (by their state? by how they have been used in the executions?).
